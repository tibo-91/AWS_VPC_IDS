...
=========================================================================================
VPC & IDS Lab 
Part 2: IDS Service mounting automatization
...
while true; do
	...
	if [[ "$ids_status" == "running" ]]; then
		...
		ssh -i ~/.ssh/$keyname \
		    -t ubuntu@$web_ipv4 \
            "ssh -i ~/.ssh/$keyname ubuntu@$ids_ipv4 -t 'wget http://perso.esiee.fr/~boiss/AWS/configure_ids.sh'; \
...
The IDS server has been configured. 
